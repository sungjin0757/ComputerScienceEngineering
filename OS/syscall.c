...
#define ENTRY 80
struct Process_Info plist[ENTRY]={0,};
/*
...
static int Sys_PS(struct Interrupt_State *state) {
    ...
    struct Kernel_Thread *kt=(&s_allThreadList)->head;
    int i;
    for(i=0;i<ENTRY;i++)
    {
    	if(kt==0)
    	...
    	plist[i].pid=kt->pid;
    	if(kt->owner !=0)
        	plist[i].parent_pid=kt->owner->pid;
    	else
    		plist[i].parent_pid=0;
       
    
    	plist[i].currCore=0;
    	
        if (kt->refCount == 2 && kt->alive)
            plist[i].status = STATUS_RUNNABLE;
    	if((kt->refCount==0 && !(kt->alive))||(kt->refCount==1 && !(kt->alive)))    		plist[i].status=STATUS_ZOMBIE;
    	if(kt->refCount==1 && kt->alive)
    		plist[i].status=STATUS_BLOCKED;
    
        plist[i].affinity = kt->affinity;
        plist[i].priority = kt->priority;
        plist[i].totalTime = kt->totalTime;
        if (kt->userContext == 0)
        {
            memcpy(plist[i].name, kt->threadName, 20);
        }
        else
        {
            memcpy(plist[i].name, kt->userContext->name, MAX_PROC_NAME_SZB);
        }
    	kt=kt->nextAll_Thread_List;
    }
    if(!Copy_To_User(state->ebx,plist,ENTRY*sizeof(struct Process_Info)))
    	return -1;
    else
    	return i;	
